import { // {{{1
  Asset, BASE_FEE, Claimant, Keypair, Memo, MemoHash, MemoReturn, MemoText, Operation,
...
function txDesc (tx) { // {{{1
  if (
    ...
    return Promise.resolve({ txId: tx.id, txSrc: tx.source_account, });

    return tx.operations().then(ops => Promise.resolve({
      amount: ops.records.find(op => op.type == 'payment' && op.asset_code == 'ClawableHexa').amount,
        txId: tx.id,
        txMemo: memo2str(tx),
        txMemoType: tx.memo_type,
        txSrc: tx.source_account,
  ...
  return Promise.resolve({
    message: 'txDesc tx UNEXPECTED', tx
  });
}
