...
function handle_stateInitial (e) { // {{{1
  return effectDesc(e).then(desc => {
    ...
    //this.log('handle_stateInitial desc', desc)

    let state = this.context.state; this.context.state = stateAnnBobSettingUp
...
function handle_stateAnnBobSettingUp (e) { // {{{1
  return effectDesc(e).then(desc => {
    ...
    let state = this.context.state; this.context.state = stateAnnBobSetup
    return state.resolve();
  });
}
