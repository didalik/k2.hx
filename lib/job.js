...
function runMonitor (opts) { // {{{1
  ...
  //console.log('runMonitor opts', opts)

  sdk.addStream(opts,
    "Bob's trading effects",
    [
      ['trade', trade]
    ],
    account.id, true
  )
  //sdk.transaction.makeSellOffer.call(this, d.kp, account, d.MA, d.XLM, '1', '1')
}
...
function startMonitor (opts) { // {{{1
  ...
  console.log('startMonitor asset', asset)
