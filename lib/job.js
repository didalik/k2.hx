...
import { makeOffer,makeRequest, } from './api.js'
import { Asset, Keypair, } from '@stellar/stellar-sdk'

let sdk, vault, accounts = {}, stateInitial = { // {{{1
  handle: handle_stateInitial,
}, stateInitial4user = { // {{{1
  handle: handle_stateInitial4user,
}

function Context (user = null) { // {{{1
// see also https://en.wikipedia.org/wiki/State_pattern
////
  if (user) {
    this.state = stateInitial4user
    this.user = user
  } else {
...
function Demo (opts) { // see also https://www.youtube.com/watch?v=y4TELgx28D4 {{{1
  ...
  return destFund(opts). // Get HEXA 900 for Cyn to run the demo.
  then(_ => {            // Get HEXA 100 for Bob to run the demo.
    opts.amount = '100'
    opts.cyn = opts.account
    opts.destKeys = opts.bobKeys
    return destFund(opts);
  }).
  then(_ => {
...
function DemoUser (opts) { // see also https://www.youtube.com/watch?v=y4TELgx28D4 {{{1
  vault ??= opts.vault
  sdk ??= hXsdk({ vault })

  sdk.addStream(opts, "Issuer's claimant effects for user",
    [['claimable_balance_claimant_created', issuerClaimant4user]], opts.issuerKeys[1]
  )
  return destFund(opts). // Get HEXA 1100 to run the demo (as Ann).
...
function handle_stateInitial4user (e) { // {{{1
  this.log('handle_stateInitial4user e', e)
...
function issuerClaimant4user (effect) { // {{{1
  this.context.state.handle.call(this, effect)
}
...
export { // {{{1
  Context, Demo, DemoSign, DemoTmReset, DemoTmUse, DemoTmUseRequest, DemoUser,
}
