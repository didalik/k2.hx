...
function addStream (opts, tag, types, id, now = false) { // {{{1
  let { streams, } = opts
  if (streams.find(s => s.tag == tag && s.id == id)) {
    opts.out('addStream DUPLICATE tag ' + tag + ' types ' + types + ' id ' + id)

    return;
  }
  map[id] ??= []
  if (map[id].find(e => e.opts === opts)) {
    opts.out('addStream DUPLICATE opts ' + opts)
  ...
  let close = effects4account.stream({
    onerror:   e => { console.error('addStream tag', tag, e); close(); throw e; },
    onmessage: e => map[id].forEach(v => {
      let pair = v.types.find(p => p[0] == e.type)
      if (pair) {
        pair[1].call(v.opts, e)
      } else {
        //console.error('addStream IGNORE', tag, e.type, id, map)
      }
    })
  })
...
function balance (account, asset_code = null) { // {{{1
  if (!asset_code) {
    return account.balances.find(e => e.asset_type == 'native').balance;
  }
  return account.balances.find(e => e.asset_code == asset_code).balance;
}
...
function hXsdk (opts = {}) {
  ...
  let target = Object.assign({ addStream, balance, networkPassphrase, sign, }, opts)
...
function pay2public (source, destination, amount) { // {{{1
  console.log('pay2public source', source,
...
function sign (kp, xdr, tag) { // {{{1
  if (!sdk.transaction[tag]({ signXDR: true, kp, xdr })) {
    return Promise.reject(new Error(`sign tag ${tag} NOT authorized`));
  }
