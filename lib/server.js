...
function injectAccount (opts) { // {{{1
  ...
  account.name = sdk.server.opts4loadAccount.name
  ...
  if (sdk?.transaction?.opts4createAccount?.creator?.account) {
    ...
    // Create account with new Keypair, startingBalance XLM 1000.0
    account.keypair = Keypair.random()
    account.startingBalance = '1000.0' // asset_type native
    return sdk.transaction.createAccount({ account, creator });
  }
  sdk.out('injectAccount - creating/getting creator...')

  opts.creator.then(_creator => { // {{{2
    creator = _creator
    let CREATOR_PK = creator.keypair.publicKey()
    sdk.out(`- loading creator.account ${CREATOR_PK} ...`)
...
function loadAccount (opts) { // {{{1
  ...
  return this.opts4loadAccount.account.promise.
    then(accountId => {
      sdk.out('loadAccount accountId ' + accountId + ' - loading...')
