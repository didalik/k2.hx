...
function createAccount (opts) { // {{{1
  ...
  return sdk.server.server.submitTransaction(tx).
    then(r => (sdk.vault.put(key, value), opts.account.resolve(destination))).
      ...
      //console.error('createAccount ERROR', e?.response?.data?.extras?.result_codes ?? e) FIXME
...
function makeBuyOffer ( // {{{1
  ...
  return sdk.server.server.submitTransaction(tx).
    then(tx => result(tx)).
    catch(e => console.error('makeBuyOffer ERROR', e.response?.data?.extras?.result_codes ?? e));
}
...
function makeSellOffer ( // {{{1
  ...
  return sdk.server.server.submitTransaction(tx).then(tx => result(tx)).
      ...
      console.error('makeSellOffer *** ERROR ***', e.response?.data?.extras?.result_codes)
