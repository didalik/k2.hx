...
import { HEX_DISPUTE, } from './api.js'
...
function changeTrust (opts, loadAccount = true) { // {{{1
  ...
  let f = account => {
    ...
    console.log('changeTrust', defaults.assets.length, 'assets for', actor, opts.recipient.id,
...
function createAccount (opts, loadAccount = true) { // {{{1
  ...
  let f = account => {
    ...
    console.log('\n\rcreateAccount - creating account for', actor, destination,
      account.last_modified_time, '...')
...
function fund (opts = null, loadAccount = true) { // {{{1
  ...
  let f = account => {
    ...
    tx.sign(kp)
    let actor = opts?.name ?? opts?.account?.name
    console.log('fund', actor, opts.destKeys[1], 'clawback', opts.clawback, 
      account.last_modified_time, '...')
...
function makeBuyOffer ( // {{{1
  ...
  let f = account => {
    ...
    console.log('makeBuyOffer', actor, selling, buying, buyAmount, price, offerId, '...')
    return submit(tx, 'makeBuyOffer', actor);
    /*
    return sdk.server.server.submitTransaction(tx).
      then(tx => result(tx)).
      catch(e => console.error('makeBuyOffer *** ERROR ***', e?.response.data ?? e));
      */
  }
  if (loadAccount) {
    return loop(kp, f);
...
function makeClaimableBalance ( // {{{1
  ...
  return sdk.server.server.submitTransaction(tx).then(txR => ({
    ...
    console.error('makeClaimableBalance *** ERROR ***', e?.response.data ?? e);
    throw e;
  });
}
