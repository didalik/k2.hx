...
let sdk; const transaction = { // handlers {{{1
  ...
  opts4declawAndFund: { // {{{2
    get(...args) { // {{{3
      return args[1] in args[0] ? Reflect.get(...args) : known_opts4declawAndFund(...args);
    }, // }}}3
  },
  // }}}2
}
...
function declawAndFund (opts) { // {{{1
  sdk ??= this.sdk // this === sdk.transaction
  this.opts4declawAndFund ??= new Proxy(opts, transaction.opts4declawAndFund)
  let defaults = this.opts4declawAndFund.defaults
}
...
function inject_default_opts4declawAndFund (opts) { // for a clawable asset issuer {{{1
  //console.log('inject_default_opts4declawAndFund this', this, 'opts', opts)
...
function known (target, prop) { // {{{1
  ...
  target[prop] = prop == 'changeTrust' ? changeTrust :
    ...
    prop == 'declawAndFund' ? declawAndFund : unknown
...
function known_opts4declawAndFund (target, prop) { // {{{1
  //console.log('known_opts4declawAndFund target', target, 'prop', prop)
  //console.log('known_opts4declawAndFund prop', prop)

  return target[prop] = inject_default_opts4declawAndFund(target);
}
