...
function createAccount (opts) { // {{{1
  ...
  return sdk.server.server.submitTransaction(tx).
    then(r => (sdk.vault.put(key, value), opts.account.resolve(destination))).
      ...
      //let codes = e?.response?.data?.extras?.result_codes
...
function fund (opts = null) { // {{{1
  ...
  sdk ??= this.sdk // this === sdk.transaction
  if (opts.remote) {
    if (opts.tag != 'fund') throw Error(`INVALID opts.tag ${opts.tag}`) // TODO complete
    let tx = TransactionBuilder.fromXDR(opts.xdr, sdk.networkPassphrase)
  ...
  let kp = Keypair.fromSecret(opts.account ? opts.accountKeys[0] : opts.issuerKeys[0])
  let sign = opts.sign
  ...
  if (sign) {
    return sign(tx.toXDR(), 'fund').then(txXdr =>
      ...
      console.error('fund.local ERROR', e.response.data); throw e
    });
  }
  return sdk.server.server.submitTransaction(tx).
