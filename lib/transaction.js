...
//    catch(e => console.error('changeTrust *** ERROR ***', e?.response?.data ?? e));
}
...
function createAccount (opts) { // {{{1
  ...
  console.log('\n\rcreateAccount - creating account for', opts.account.name, destination, '...')

  return sdk.server.server.submitTransaction(tx).
    then(r => (sdk.vault.put(key, value), opts.account.resolve(destination))).
    catch(e => {
      //console.error('createAccount *** ERROR ***', e?.response?.data ?? e) FIXME
...
function fund (opts = null) { // {{{1
  ...
  if (sign) {
    return sign(tx.toXDR(), 'fund').then(txXdr =>
      submit(TransactionBuilder.fromXDR(txXdr, sdk.networkPassphrase), 'fund', actor, 5, 100)
    ).catch(e => console.error('fund', actor, '*** ERROR ***', e?.response?.data ?? e));
  }
  return sdk.server.server.submitTransaction(tx).
    catch(e => console.error('fund *** ERROR ***', e?.response.data ?? e));
}
...
function submit (tx, fn, actor, left = 5, delayMs = 5000, log = console.error) { // {{{1
  return sdk.server.server.submitTransaction(tx).catch(e => {
    log('submit', fn, 'for', actor, '*** ERROR ***', e?.response?.data ?? e, 'left', left)
    ...
    return new Promise(resolve => setTimeout(resolve, delayMs)).then(_ => {
      log('submit', fn, 'for', actor, 'on-chain?..')
      return sdk.server.server.transactions().transaction(txHash).call();
    }).then(txCheck => {
      log('submit', fn, 'for', actor, 'txCheck', txCheck)
      return txCheck; // Transaction was successful!
    }).catch(e => {
      if (!(e?.response.status === 404)) {
        throw e;
      }
      log('submit', fn, 'for', actor, 'NOT FOUND on-chain, re-submitting...')
