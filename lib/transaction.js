...
import { HEX_DISPUTE, asset, } from './api.js'
...
function breakDeal (opts) { // {{{1
  ...
  return opts.sign(tx.toXDR(), 'breakDeal').then(txXdr => // {{{2
    ...
    then(r => opts.nolog || opts.log(r)).catch(e => {
      console.error('breakDeal.local ERROR', e?.response?.data ?? e); throw e
    });
  // This transaction produces 2 effects for issuer (same as closeDeal):
  // - account_credited ClawableHexa amount; then
  // - account_debited HEXA amount.
  // }}}2
}

function changeTrust (opts, loadAccount = true) { // {{{1
  ...
  let account = opts.recipient, kp = Keypair.fromSecret(opts.recipientKeys[0])
  let f = account => {
    let tx = new TransactionBuilder(account, { fee: BASE_FEE }).setNetworkPassphrase(sdk.networkPassphrase)
    for (let a of opts.acls ?? defaults.assets) {
      tx.addOperation(Operation.changeTrust({
        asset: new Asset(a[0], opts.issuer.id),
        limit: a[1]
      }))
    }
    tx = tx.setTimeout(60).build()
    tx.sign(kp)
    let actor = opts?.name ?? opts?.account?.name
    let length = opts.acls ? opts.acls.length : defaults.assets.length
    sdk.out('changeTrust ' + length + ' assets for ' + actor + ' ' +
      opts.recipient.id + ' ' + account.last_modified_time + '...')
    return submit(tx, 'changeTrust', actor);
  }
  if (loadAccount) {
    return loop(kp, f);
  }
...
function closeDeal (opts) { // {{{1
  ...
  // This transaction produces 2 effects for issuer (same as breakDeal):
  // - account_credited ClawableHexa amount; then
  // - account_debited HEXA amount.
  // }}}2
}

function createAccount (opts, loadAccount = true) { // {{{1
  sdk ??= this.sdk // this === sdk.transaction
  this.opts4createAccount = new Proxy(opts, transaction.opts4createAccount)
  let account = opts.creator.account
  let actor = opts.account.name
  let defaults = this.opts4createAccount.defaults
  let destination = opts.account.keypair.publicKey()
  let kp = opts.creator.keypair
  ...
  let f = account => {
    let tx = new TransactionBuilder(account, { fee: BASE_FEE }).
      ...
      setTimeout(60).build();
    tx.sign(kp, opts.account.keypair)
    sdk.out('createAccount - creating account for ' + actor + ' ' + destination + ' ' +
      account.last_modified_time + '...')
    return submit(tx, 'createAccount', actor, 5, 100, null).
...
function disputeBrokenDeal (opts) { // {{{1
  let HEXA = new Asset('HEXA', opts.issuer.id)
  let kp = Keypair.fromSecret(opts.destKeys[0])
  let plaintiff = opts.account
  let claimants = [ 
    new Claimant(opts.issuer.id,
      opts.validity == '0' ? Claimant.predicateUnconditional()
  ...
  let amount = HEX_DISPUTE
  return makeClaimableBalance.call(opts.sdk, claimants, plaintiff, kp, HEXA, amount,
    [], Memo.hash(opts.breakDealTxId)
  );
}

function fund (opts = null, loadAccount = true) { // {{{1
  if (opts === null) { // noop
  ...
  let p2p = opts?.pay2public
  let account = p2p ? p2p.srce : opts.account ? opts.account : opts.issuer
  let source = p2p ? p2p.srce.id : opts.issuerKeys[1]
  let a = asset(opts)
  let destination = opts.destKeys[1]
  let amount = opts.amount
  let kp = p2p ? p2p.kp : Keypair.fromSecret(opts.account ? opts.destKeys[0] : opts.issuerKeys[0])
  let sign = opts.sign
  let tf = { asset: a, source, trustor: destination, flags: { authorized: true, } }
  let f = account => {
    if (opts.clawback === false) {
      tf.flags.clawbackEnabled = false
    }
    let tx = p2p ? new TransactionBuilder(account, { fee: BASE_FEE }).
      setNetworkPassphrase(sdk.networkPassphrase).
      addOperation(Operation.payment({ asset: a, destination, amount, source }))
      : new TransactionBuilder(account, { fee: BASE_FEE }).
      setNetworkPassphrase(sdk.networkPassphrase).
      addOperation(Operation.payment({ asset: a, destination, amount, source })).
    ...
    tx.sign(kp)
    let actor = opts?.name ?? opts?.account?.name
    sdk.out('fund ' + actor + ' ' + opts.destKeys[1] + ' clawback ' + opts.clawback + ' ' + 
      account.last_modified_time + '...')

    if (sign) {
      return sign(tx.toXDR(), 'fund').then(txXdr => {
        return submit(
          TransactionBuilder.fromXDR(txXdr, sdk.networkPassphrase), 'fund', actor, 5, 100, null
        );
      });
    }
    return submit(tx, 'fund 2', actor, 5, 100, null);
  }
  if (loadAccount) {
    return loop(kp, f);
  }
...
function known (target, prop) { // {{{1
  ...
  target[prop] = 
    ...
    prop == 'disputeBrokenDeal' ? disputeBrokenDeal :
    prop == 'fund' ? fund :
...
function loop (kp, f, left = 9, log = console.error) { // {{{1
  let pk = kp.publicKey()
  return sdk.server.server.loadAccount(pk).then(account => f(account)).catch(e => {
    log && log('loop', pk, '*** ERROR ***',
      e?.response?.data?.extras?.result_codes ?? e?.response?.data ?? e, 'left', left
    )
    if (left < 1) {
      throw e;
    }
    if (e?.response?.data?.extras?.result_codes?.transaction == 'tx_bad_seq') {
      return loop(kp, f, --left);
    }
    throw e;
  });
}
...
function makeBuyOffer ( // {{{1
  kp, account, selling, buying, buyAmount, price, offerId = 0, loadAccount = true
) {
  sdk ??= this
  let f = account => {
    ...
    tx.sign(kp)
    let actor = kp.publicKey()
    sdk.out('makeBuyOffer ' + actor + ' ' + selling + ' ' + buying + ' ' + buyAmount + ' ' +
      price + ' ' + offerId + '...')
    return submit(tx, 'makeBuyOffer', actor);
  }
  if (loadAccount) {
    return loop(kp, f);
  }
...
function makeClaimableBalance ( // {{{1
  claimants, maker, kp, asset, amount, ops = [], memo = null, loadAccount = true
) {
  sdk ??= this
  let f = account => {
    let tx = new TransactionBuilder(maker, // increasing the maker's
...
function makeSellOffer ( // {{{1
  ...
  tx.sign(kp)
  return sdk.server.server.submitTransaction(tx).then(tx => result(tx)).
    catch(e => {
      console.error('makeSellOffer *** ERROR ***', e?.response.data ?? e)
      console.trace(account.id, selling, buying, amount, price)
...
function submit (tx, fn, actor, left = 5, delayMs = 5000, log = console.error) { // {{{1
  return sdk.server.server.submitTransaction(tx).catch(e => {
    log && log('submit', fn, 'for', actor, '*** ERROR ***', 
      e?.response?.data?.extras?.result_codes ?? e?.response?.data ?? e, 'left', left)
    if (left < 1 || !(e?.response.status === 504)) {
      throw e;
    } // else tx submission timed out, but could be successful anyway:
    const txHash = tx.hash().toString("hex")
    return new Promise(resolve => setTimeout(resolve, delayMs)).then(_ => {
      log && log('submit', fn, 'for', actor, 'on-chain?..')
      return sdk.server.server.transactions().transaction(txHash).call();
    }).then(txCheck => {
      log && log('submit', fn, 'for', actor, 'txCheck', txCheck)
      return txCheck; // Transaction was successful!
    }).catch(e => {
      if (!(e?.response.status === 404)) {
        throw e;
      }
      log && log('submit', fn, 'for', actor, 'NOT FOUND on-chain, re-submitting...')
      return submit(tx, fn, actor, --left, delayMs, log);
    });
  });
}
