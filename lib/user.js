...
import { effectDesc, makeRequest, } from './api.js'
...
function Demo (opts) { // see also https://www.youtube.com/watch?v=y4TELgx28D4 {{{1
  ...
  opts.context = new Context(stateInitial, 'user')
  ...
  then(takingRequest => deal.call(opts, takingRequest)). // request taken
...
function deal (takingRequest) { // {{{1
  this.log('deal takingRequest', takingRequest, 'this', this)
...
function handle_stateInitial (e) { // {{{1
  /*if (++stateInitial.count < 3) {
    return Promise.resolve();
  }*/
  this.log('handle_stateInitial e', e) //, 'this.context.state', this.context.state)
  return Promise.resolve();

  let state = this.context.state; this.context.state = stateCynAnnDeal
...
function rs4d (opts) { // Request red snapper for dinner. {{{1
  ...
  opts.validity = '0'
  Object.assign(stateInitial, Promise.withResolvers(), { count: 0 })
